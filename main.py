...
from gameplay import Camera, UpBar, Hourglass, Decision, Build_Menu, BuildItem, Timer, SideMenu ,Stats,Player
...
# Ustawienie flag akceleracji sprzętowej i podwójnego buforowania
...
class Game:
    def __init__(self):
        ...
        self.map.texture()
        self.map.generate()
        self.up_bar = UpBar(screen,self)
        self.klepsydra1 = Hourglass(screen,self)
        self.dec = Decision(screen)
        self.bm = Build_Menu(screen)
        self.timer = Timer(screen,self)
        self.sd = SideMenu(screen,self)
    ...
    def run(self):
        ...
        while True:
            ...
            self.camera.keybord()
            self.map.draw()
            self.map.zajmij_pole(player=player)
            ...
            if Build_Menu.build_stauts:
                ...
                for item in self.allItem:
                    item.draw()
                    item.buy(player=player)
            if not self.bm.build_stauts:
                self.dec.click(player=player)

            self.klepsydra1.draw()
            if player.wyb == False:
                self.klepsydra1.turn(player=player)
                
            if player.wyb:
                self.dec.draw(player=player)
            self.timer.update()
            FPS()
            pygame.display.flip()
            clock.tick(max_tps)
